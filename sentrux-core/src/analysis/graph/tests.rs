...
mod tests {
    ...
    fn call_edges_blocked_without_import() {
        ...
        assert_eq!(
            gr.call_edges.len(),
            0,
            "call edge without import should be blocked"
        );
    }
    ...
    fn rust_crate_prefix_resolves() {
        ...
        assert_eq!(
            gr.import_edges.len(),
            1,
            "use beemem::capture should resolve to src/capture.rs, got {:?}",
            gr.import_edges
        );
    ...
    fn rust_crate_prefix_no_stdlib_match() {
        ...
        assert_eq!(
            gr.import_edges.len(),
            0,
            "std:: import should not match any project file"
        );
    ...
    fn tier3_no_self_edges() {
        ...
        assert!(
            ...
            "Self-imports should not produce edges"
        );
    ...
    fn tier3_unknown_target_no_edge() {
        ...
        assert!(
            gr.import_edges.is_empty(),
            "Unknown target should produce no edge"
        );
    ...
    fn tier3_no_root_returns_empty() {
        ...
        assert!(
            gr.import_edges.is_empty(),
            "None scan_root should return empty"
        );
    }
}
