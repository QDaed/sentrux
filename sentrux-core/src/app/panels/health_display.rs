...
pub fn draw_health_section(ui: &mut egui::Ui, report: &HealthReport, tc: &ThemeConfig) {
    ...
    draw_root_cause_row(
        ui,
        "equality",
        format!("G={:.2}", raw.complexity_gini),
        scores.equality,
        tc,
        row_h,
        &font,
    ...
    draw_root_cause_row(
        ui,
        "redundancy",
        format!("{}", (raw.redundancy_ratio * 10000.0).round() as u32),
        scores.redundancy,
        tc,
        row_h,
        &font,
...
fn draw_quality_signal(ui: &mut egui::Ui, report: &HealthReport, tc: &ThemeConfig) {
    ...
    let bar_str: String = (0..total_blocks)
...
fn draw_flagged_files(ui: &mut egui::Ui, report: &HealthReport, tc: &ThemeConfig, row_h: f32) {
    ...
    draw_flagged_list(
        ui,
        "HOTSPOTS (fan-in)",
        &report.hotspot_files,
        warn_color,
        row_h,
    );
}

fn draw_flagged_list(
    ui: &mut egui::Ui,
    title: &str,
    items: &[crate::metrics::FileMetric],
    color: egui::Color32,
    row_h: f32,
) {
    if items.is_empty() {
...
fn draw_unstable(ui: &mut egui::Ui, report: &HealthReport, tc: &ThemeConfig, row_h: f32) {
    ...
    ui.add_space(3.0);
    ui.label(
        egui::RichText::new("UNSTABLE (I>0.8)")
            .monospace()
            .size(8.0)
            .color(color),
    );
    for m in &unstable {
        ...
        let text = format!(
            "  {} I:{:.2} out:{} in:{}",
            name, m.instability, m.fan_out, m.fan_in
        );
        let (rect, resp) = ui.allocate_exact_size(
            egui::vec2(ui.available_width(), row_h),
            egui::Sense::hover(),
